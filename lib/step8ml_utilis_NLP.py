...
def fit_dl_classif(X_train, y_train, X_test, encode_y=False, dic_y_mapping=None, model=None, weights=None, epochs=100, batch_size=256):
    ...
    predicted = [dic_y_mapping[np.argmax(predicted_prob)] for pred in predicted_prob] if encode_y is True else [np.argmax(predicted_prob)]
...
def word_clustering(corpus, nlp=None, ngrams=1, grams_join=" ", lst_ngrams_detectors=[], n_clusters=3):
    ...
    if nlp is None:
        print("--- training W2V---")
        lst_corpus, nlp = fit_w2v(corpus, ngrams=ngrams, grams_join=grams_join, lst_ngrams_detectors=lst_ngrams_detectors, min_count=1, window=20, sg=0, epochs=100)
