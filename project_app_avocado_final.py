...
try:
    ...
    if choice == 'Nhìn chung về bơ hass':
        ...
        # tinh RMSE, MAE, R^2
        model = model_rf
        Train_Dataset_RMSE = reg.mean_squared_error(y_true=y_train, y_pred=model.predict(X_train), squared=False)
        Test_Dataset_RMSE = reg.mean_squared_error(y_true=y_test, y_pred=model.predict(X_test), squared=False)
        ...
        fig1  = reg.evaluate_regr_model(y_train, y_pred, figsize=(25,5))

        fig2 = reg.evaluate_regr_model(y_test, predicted, figsize=(25,5))
        ...
        output = st.empty()
        with st_capture(output.code):
            print(reg.evaluate_regr_model(y_train, y_pred, figsize=(25,5)))
        st.write(" ")
        ...
        output = st.empty()
        with st_capture(output.code):
            print(reg.evaluate_regr_model(y_test, predicted, figsize=(25,5)))
        st.write(" ")
        ...
        choice1 = st.sidebar.radio("Chọn model bạn muốn ?",("Custom Model", "Prophet", 'Neural Network','Autoregressive', 'SarimaX', 'Random Walk'))
        ...
        if content_ == "Admin":
            ...
            if st.sidebar.button("Hãy nhấn vào tôi đi nào 🤡"):
                ...
                ### 1.7. Model Desing & Testing (Neural Network)
                    ...
                    with st_capture(output.code):
                        # -> There is a slight trend and it's linear ("additive")
                        print(ser.fit_lstm(ts_train, ts_test, model, exog=None, s=s, epochs=100, conf=0.20, figsize=(15,10)))
                    st.write(" ")
                    ...
                    with st_capture(output.code):
                        # -> There is a slight trend and it's linear ("additive")
                        print(ser.forecast_lstm(data_processing, model, conf=0.20, end=end, zoom=30, figsize=(15,5)))
                    st.write(" ")
                ...
                ### 1.6. - Model Desing & Testing (Autoregressive)
                    ...
                    with st_capture(output.code):
                        # -> There is a slight trend and it's linear ("additive")
                        print(ser.tune_expsmooth_model(ts_train, s=s, val_size=0.2, scoring=reg.metrics.mean_absolute_error, top=4, figsize=(15,5)))
                    st.write(" ")
